...
async def async_setup_entry(
    ...
    async_add_entities([
        ...
        PoolPalEmptyCmLevel(hass, entry),
...
class PoolPalEmptyCmLevel(_CalibrationEntity):
    ...
    _attr_mode = NumberMode.BOX
    _attr_name = "Empty Level"

    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        super().__init__(hass, entry)
        self._attr_unique_id = f"{DOMAIN}_{entry.entry_id}_empty_cm_level"
        self._attr_native_value = entry.data.get(CONF_EMPTY_CM_LEVEL, DEFAULT_EMPTY_CM_LEVEL)
        hass.data[DOMAIN][entry.entry_id][CONF_EMPTY_CM_LEVEL] = self._attr_native_value
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if state is not None and state.state not in (STATE_UNAVAILABLE, STATE_UNKNOWN):
            try:
                ...
                pass
        self.hass.data[DOMAIN][self._entry.entry_id][CONF_EMPTY_CM_LEVEL] = self._attr_native_value
